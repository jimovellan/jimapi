...
namespace jim.test.Services
{
    ...
    public class UserServiceTest
    {
        ...
        [TestMethod]
        public async Task When_IAddNewUserToConnection_CanIFindIt()
        {
            var userId = Guid.NewGuid().ToString();
            var nameId = "Name1";
            var user = User.Create(nameId, userId);

            await _sut.AddUserToConectionAsync(user);
            var userFinded = await _sut.GetUserAsync(userId);

            Assert.AreEqual(user, userFinded);
