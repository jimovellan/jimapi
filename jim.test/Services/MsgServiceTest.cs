...
namespace jim.test.Services
{
    ...
    public class MsgServiceTest
    {
        ...
        public async Task WhenSendAMessageTo_A_User_Should_NotificateMsg_OneTime()
        {
            ...
            _userServiceMock.Setup(s => s.GetUserAsync(It.IsAny<string>())).ReturnsAsync(userMock);

            await _sut.AddMessageAsync(msgMock);

            _notificationService.Verify(x => x.SendMessageToUserAsync(It.IsAny<MsgDto>()), Times.AtLeastOnce);
